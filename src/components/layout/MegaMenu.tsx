...
export function MegaMenu({ isOpen, onClose }: MegaMenuProps) {
    ...
    return (
        <div className="absolute top-full left-0 w-full bg-theme-bg-surface shadow-xl border-t border-theme-border z-50 animate-in fade-in slide-in-from-top-2 duration-200" onMouseLeave={onClose}>
            <div className="w-full py-8 px-4 lg:px-10">
                <div className="grid grid-cols-1 lg:grid-cols-7 gap-8 lg:gap-10">
                    <div className="lg:col-span-5 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-8">
                                ...
                                <div key={category.name} className="space-y-4">
                                    ...
                                    <ul className="space-y-2">
                                        {category.subcategories.map((sub) => (
                                            <li key={sub}>
                                                <Link href={`/category/${category.name.toLowerCase().replace(/ /g, "-")}/${sub.toLowerCase().replace(/ /g, "-")}`} className="text-theme-text-secondary hover:text-theme-text-primary hover:underline text-sm transition-colors" onClick={onClose}>
                                                    {sub}
                                                </Link>
                                            </li>
                                        ))}
